...
`103.206.217.28:5622`、`103.206.217.28:6662` 和旧目标
`103.206.217.41:6664`。客户端会把这些目标映射到 Worker 上各自固定的 relay
路径。未知 IP、端口和 Worker 路径仍会被拒绝。

## 9. 安全要求

- 不要把原始 `RYLUX_RELAY_TOKEN` 写进 Java、Gradle、Worker 代码或 APK；
- 不要把登录 session token 和 relay 凭证提交到 Git；
- relay 目标只能是 Worker 白名单中的 `103.206.217.28:5622`、
  `103.206.217.28:6662` 或旧目标 `103.206.217.41:6664`；新增目标必须同时经过
  实测确认并加入客户端与 Worker 的精确白名单，绝不能改成任意目标代理；
